...
const getProblemUrlCell = (problem, width) =>
  `<td style="width: ${width || 45}%;"><a target="_blank" href=${
    problem.url
  } title="${problem.name}" class="problem-full-name"><small>${
    problem.name
  }</small></a></td>`;
...
const getProblemLevelCell = (problem, width) =>
  `<td style="width: ${width || 10}%;"><small>${decorateProblemLevel(
    problem.level
  )}</small></td>`;
...
const createScheduleProblemRecord = (problem) => {
  ...
  const htmlTag = `\
    <tr style="vertical-align:middle">\
        ...
        ${getProblemProgressBarCell(problem, 10)}\
        ${getProblemLevelCell(problem, 10)}\
        <td style="width: 10%;"><small>${
          months[nextReviewDate.getMonth()]
        } ${nextReviewDate.getDate()}</small></td>\
...
export const renderReviewTableContent = (problems, page) => {
  ...
  if (page === store.toReviewMaxPage)
    nextButton0DOM.setAttribute("disabled", "disabled");
  if (page !== store.toReviewMaxPage)
    nextButton0DOM.removeAttribute("disabled");
...
export const renderScheduledTableContent = (problems, page) => {
  ...
  if (page === store.scheduledMaxPage)
    nextButton1DOM.setAttribute("disabled", "disabled");
  if (page !== store.scheduledMaxPage)
    nextButton1DOM.removeAttribute("disabled");
...
export const renderCompletedTableContent = (problems, page) => {
  ...
  if (page === store.completedMaxPage)
    nextButton2DOM.setAttribute("disabled", "disabled");
  if (page !== store.completedMaxPage)
    nextButton2DOM.removeAttribute("disabled");
...
export const renderUndoButton = async () => {
  if (await hasOperationHistory()) {
    Array.prototype.forEach.call(undoButtonDOMs, (btn) =>
...
export const renderAll = async () => {
  ...
  const problems = Object.values(await getAllProblems()).filter(
    (p) => p.isDeleted !== true
  );
